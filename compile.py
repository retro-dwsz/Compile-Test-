...
CLANG_DEBUG_FLAGS = ["-O0", "-Wno-unused-but-set-variable"]
CLANG_RELEASE_FLAGS = ["-O3", "-Wno-unused-but-set-variable"]

MSVC_DEBUG_FLAGS = ["/Od", "/Zi"]
MSVC_RELEASE_FLAGS = ["/O2 /std:c++20"]
...
def compile_with_clang(output_name, flags) -> None:
    base_cmd = ["clang++", SOURCE_FILE, "-o", os.path.join(OUTPUT_DIR, output_name)]
    cmd = base_cmd + flags
    run_command(cmd)
...
def compile_with_msvc(output_name, flags) -> None:
    ...
    base_cmd = ["cl", SOURCE_FILE, "/EHsc", "/nologo", "/Fe:" + os.path.join(OUTPUT_DIR, output_name)]
    cmd = base_cmd + [f for f in flags] + ["/Fo" + os.path.join(OUTPUT_DIR, "obj_") + output_name.replace(".exe", ".obj")]
    run_command(cmd)
